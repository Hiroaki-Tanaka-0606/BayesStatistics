...
//calculate posterior distribution function (model: one Lorentz)
	...
	for(i=0; i<sigma_split+1; i++){
		...
		for(j=0; j<mu_split+1; j++){
			...
			for(k=0; k<num_data; k++){
				//add log(p(data_list[k]; sigma, mu))=log_sigma-log(pi*((data_list[k]-mu)^2+sigma^2))
				log_posterior_distribution[i][j]+=log_sigma-log(M_PI*(pow(data_list[k]-mu,2)+pow(sigma,2)));
			}
			if(i==0 && j==0){
				//first term
